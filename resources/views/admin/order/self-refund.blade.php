...
<div class="am-tabs am-tabs-d2 am-margin" data-am-tabs>
    ...
    <a style="color:blue;line-height:37px;margin-left: 75px;" href="{{url("/admin/order/refund")}}">申请退款的订单</a>
    <a style="color:black;margin-left: 160px;" href="{{url("/admin/order/refunda")}}">退款完成的订单</a>
    </div>
        {{--已完成的所有订单--}}
                                ...
                                <div class="order-right">
                                    ...
                                    <div class="move-right">
                                        <li class="td td-status">
                                            <div class="item-status" style="margin-top: -43px;">
                                                <p class="Mystatus">退款原因</p>
                                                <p>{{$v->reason == 0 ? '拍错了': ($v->reason == 1 ? '不想要了': ($v->reason == 2 ? '没钱了'  : ($v->reason == 3 ? '卖完后悔了': '其他原因'))) }}</p>
                                                <p>申请时间{{date('Y-m-d H:i:s',$v->applytime)}}</p>
                                            </div>
                                        </li>
                                        <li class="td td-change">
                                            <button class="am-btn am-btn-danger anniu tongyitui" id="one" od_id="{{$v->od_id}}">
                                                同意退款</button>
                                        </li>
                                    </div>
                                </div>
...
<script>
    ...
    $(".tongyitui").click(function(){
       var od_id = $(this).attr('od_id');

       //发送ajax
        $.ajax({
            type:'post',
            url: '/admin/order/tongyitui',
            ...
            success:function(data){
                ...
                if(data == 1){
                    ...
                    layer.msg('退款失败',{icon:5,time:2000});
                }
            },
            error:function(){
