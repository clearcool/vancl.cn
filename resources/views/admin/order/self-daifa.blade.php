...
<div class="am-tabs am-tabs-d2 am-margin" data-am-tabs>
    ...
    <div style="width: 100%;height: 40px;background:lightgray;">
        ...
        <a style="color:blue;margin-left: 160px;" href="{{url("/admin/order/daifa")}}">待发货订单</a>
                ...
                @foreach($detail as $k=>$v)
                        <div class="order-status5">
                            <div class="order-title">
                                ...
                                <span>店铺：{{$v->sname == 'Vancl' ? '自营' : $v->sname}}</span>
                            </div>
                                ...
                                <div class="order-right">
                                    ...
                                    <div class="move-right">
                                        ...
                                        <li class="td td-change">
                                            <div class="am-btn am-btn-danger anniu fahuo" od_id="{{$v->od_id}}" id="one">
                                                马上发货</div>
                                        </li>
                                    </div>
                                </div>
...
<script>
    ...
    $.ajax({
    type:'post',
        url: '/admin/order/modifyfahuo',
        data:{'od_id':od_id,'_token':'{{ csrf_token() }}'},
        success:function(data){
            //判断
            if(data == 1){
                layer.msg('发货成功!', {icon: 6, time: 2000});
                $("#one").parents(".order-status5").remove();
            }else{
                layer.msg('发货失败',{icon:5,time:2000});
            }
        },
        error:function(){
